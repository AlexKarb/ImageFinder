export class RESPONSE {
   ...
   this.requestURL = this.root + `&q=${this.valueOfSearch}&page=${this.page}&per_page=${this.perPage}`;
        
    };
  
 
    controlEndOfColection() {
        this.endOfColection = (Math.ceil(this.quantityOfResponses / this.perPage) === this.page  || Math.round(this.quantityOfResponses / this.perPage) === 0);       
};
    ...
    updateRequestURL() {
     this.requestURL = this.root + `&q=${this.valueOfSearch}&page=${this.page + 1}&per_page=${this.perPage}`;
}
